...
jq -cnS --arg started "$STARTED_AT" --arg install "$install_at" --arg setup "$setup_at" --arg pid "$proxy_pid" --arg start "$proxy_start" --arg config "$config_sha" --arg uuid "$uuid_sha" --arg key "$key_sha" --arg ownership "$ownership_sha" --argjson creation "$creation" --argjson release "$release" '{config_sha256:$config,creation_provenance:$creation,install_at:$install,key_sha256:$key,ownership_sha256:$ownership,proxy_pid:$pid,proxy_start_tick:$start,release_identity:$release,setup_at:$setup,started_at:$started,uuid_sha256:$uuid}' | tr -d '\n' > "${SBXR_OPERATOR_STATE_DIR}/08-private.json"
