...
When setting `TMPDIR` for SBXR validation, use a temporary subdirectory of the
local run directory. Remove disposable fixtures when the run finishes; retain
useful observations and handoffs there. `.scratch/` stays ignored by Git. Keep
