...
3. Add the exact release-specific `install.sh`, then build `release-index.json` only from it and those exact four regular archives. Require schema `1`, product `sbxr`, repository `albertloky/SBXR`, the exact version, sequence, tag, commit, State schema, minimum updater schema, exact five roles and fixed names, calculated sizes, and calculated SHA-256 values. Refuse caller-supplied digests, missing, duplicate, extra, linked, occupied, changed, or oversized material.
