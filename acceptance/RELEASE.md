...
This procedure applies only to one exact six-asset root-runtime Release Identity under ADR-0010. It does not qualify an identity with a client-facing change.
...
4. Compare the exact commit with `v1.0.6` and refuse this qualification policy if the fixed client-facing Connection Profile, Subscription Publication, or Client Access outcome files changed.
...
7. Mark Module, Seam, and Integrated Verification `Passed`. Mark Integrated Ubuntu Verification, Codex Live Acceptance, and Owner Acceptance `Not required` because ADR-0010 permits package-test-only qualification for privilege and service-identity changes. State that no Integrated Ubuntu Verification, live VPS, provider, maintained-client, or Owner evidence was performed.
