...
8. Any changed asset, attestation, repository, tag, commit, release-index digest, required check, or client-facing change invalidates the record and blocks stable publication. Stable publication must reverify the same record and public bytes, prove latest and pinned `install.sh` equality, scan all six assets, and run the ordinary command through the supported non-root Ubuntu Owner Console before activating the README command. Release `v1.0.4` first passed this gate on workflow run `31676900925`.
