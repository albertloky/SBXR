...
func writeAutomatedAcceptanceRecord(options acceptanceOptions, recordedAt time.Time) error {
	...
	fmt.Fprintln(&record)
	fmt.Fprintln(&record, "Status: Qualified - installer-only automated exception")
	...
	fmt.Fprintln(&record, "Stable result code: RELEASE-INSTALLER-AUTOMATED-QUALIFICATION")
	fmt.Fprintln(&record)
	fmt.Fprintln(&record, "| Stage | Status | Evidence |")
	fmt.Fprintln(&record, "|---|---|---|")
	fmt.Fprintln(&record, "| Module Verification | Passed | Complete suites, hostile fixtures, supported race checks, and static checks |")
	fmt.Fprintln(&record, "| Seam Verification | Passed | Native Ubuntu seams, exact public HTTPS release verification, and Sigstore attestations |")
	fmt.Fprintln(&record, "| Integrated Verification | Passed | Complete install, reclamation, recovery, Diagnostics, and Complete removal composition |")
	fmt.Fprintln(&record, "| Codex Live Acceptance | Not required | ADR-0007 installer-only scope; no live VPS evidence claimed |")
	fmt.Fprintln(&record, "| Owner Acceptance | Not required | No client-facing change; no maintained client evidence claimed |")
	fmt.Fprintln(&record)
	fmt.Fprintln(&record, "No live VPS, provider, maintained-client, or Owner evidence was performed.")
	fmt.Fprintln(&record)
	...
	fmt.Fprintln(&record)
	fmt.Fprintln(&record, "Any asset, attestation, repository, tag, commit, release-index digest, required check, or client-facing change invalidates this record.")
