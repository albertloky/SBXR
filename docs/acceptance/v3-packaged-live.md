...
For the first subscription clean-install scope, `lifecycle-menu` replaces source upgrade and two-release update/recovery scenarios. It proves the actual zero-argument packaged menu reaches Check/Update/Recover, uses explicit confirmation, safely reports no update/no recovery, and refuses clean-install-only update targets without replacement. The Acceptance Record marks incoming source upgrades and two-release update/recovery **Not applicable**, never Passed. `enable-schema1` must start from the candidate's supported setup; protected-state editing or unsupported migration cannot supply that state. All automated lifecycle safety coverage remains required.
