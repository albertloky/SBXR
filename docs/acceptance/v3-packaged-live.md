...
request the existing GitHub Ubuntu amd64 outside-client probe. Write a root-owned,
regular, one-link `0600` `outside-request.json` in the evidence directory. Its exact
canonical fields, in order, are `deadline_unix` (copy from `request.json`),
`qualification_manifest_sha256`, `request_id` (`probe-1` for `baseline-clean`,
`probe-2` for `baseline-postcommit`), `scenario_id`, and
`schema:"sbxr-v3-outside-probe-request-v1"`; no trailing newline is allowed. The
collector reuses `v3-packaged-live.sh`'s official DEB/key verification, confirmed
Show client configuration, tmpfs-only client configuration/captures, outside
traffic comparison, scans, and cleanup. It cannot run setup/removal or arbitrary
operator commands. The probe has only the scenario's remaining time. Read the
protected `outside-reply-<scenario>.json` and incorporate the actual observations
...
For `identity-unavailable`, a temporary INPUT rejection limited to the recorded
IPv4 and TCP `8443` can exercise an independent retrieval fault. Place it before
matching loopback/established/serving ACCEPT rules, with an exact qualification
comment outside the `sbxr-subscription` ownership namespace. Prove both outside
retrieval and the local public-IP HTTPS check fail while proxy traffic remains
healthy. Keep renewal evidence healthy and certificate bytes unchanged. Drive
reviewed rotation, prove serving stopped and the replacement proxy working via
confirmed Show client configuration fallback, and prove the separate subscription
fault. Remove only the injected rule. Require the reviewed runtime-only Repair
subscription Plan before recovery; it must not issue a certificate. Source review
