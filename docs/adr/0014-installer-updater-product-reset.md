...
The Installer-Updater release line begins at `v2.0.0`, sequence `17`. Stable publication requires the packaged two-release procedure originally at [`acceptance/RELEASE.md`](../acceptance/historical/installer-updater-release.md). The installed executable accepts only `sudo sbxr`; any argument is refused. Repository-only `cmd/sbxr-release` is not an Owner command.
