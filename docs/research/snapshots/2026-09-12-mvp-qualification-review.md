...
The controlling simplification principle is global AGENTS.md (`/Users/Albert/.codex/AGENTS.md:1`, local at the review date):

> Default to not adding hashes, freezing contracts, creating baselines, or introducing gates.

Its next paragraph requires a concrete failure scenario and an explanation of why Git, versioning, primary keys, transactions, unique constraints, the type system or ordinary tests do not already prevent it. [Repository AGENTS.md](../../../AGENTS.md#L27) additionally calls for proportionate changes, tests at the affected boundary, reuse of valid results, and stopping when validation uncovers a separate investigation.

The 25 scenarios are the current **v4 clean-install repair qualification policy**, not a decomposition into 25 product features. [ADR-0022](../../adr/0022-two-issuance-repair-qualification.md#L36) expressly retains 25 live scenarios, 29 automated-only scenarios and two automated-only lifecycle checks. [attemptScenarios](../../../cmd/sbxr-release/qualification_scope.go#L132) derives the current selection from a larger historical matrix. Those old requirements were explicitly approved at the time; their existence is not evidence of an unauthorized implementation. The current request is a reason to reassess them.
...
All exact declarations and file links are in the catalog. No correctness claim is made for every individual body. In particular, the private [Subscription Serving README](../../../internal/proxyinstallation/subscriptionserving/README.md#L76) still describes an old implementation slice with enablement disabled, whereas current [enablement code](../../../internal/proxyinstallation/subscription.go#L67) and the root module guide describe implemented enablement. Treat that section as stale incremental documentation, not a missing-feature requirement.
...
Scenario names and purposes are cross-checked against the [operator scenario map](https://github.com/albertloky/SBXR/blob/0859e964b66d10deb5768a372b09ca5903332553/.github/scripts/v3-operator/README.md#L191), [scenario checks](../../../cmd/sbxr-release/qualification_recurring.go#L400) and [detailed procedures](../../acceptance/historical/v4-operator-procedures.md#L163).
...
3. **Evidence bookkeeping is repeatedly enforced.** [requiredV3Checks](../../../cmd/sbxr-release/qualification_recurring.go#L367) repeats common checks and [validRecurringEvidence](../../../cmd/sbxr-release/qualification_recurring.go#L225) enforces timing, ordering and digest links. Recheck mutable product state when an action can change it; consolidate attempt-wide facts and remove redundant digest ceremony. Do not infer that checking an invariant once covers later mutations.
...
3. **The first clean-install scope does not require proving a nonexistent incoming upgrade.** [Software Lifecycle's guide](../../../internal/softwarelifecycle/README.md#L62) and [scenario 19](../../acceptance/historical/v4-operator-procedures.md#L993) distinguish future updates from current no-op/refusal behavior. Do not add an update route merely to satisfy a historical larger matrix.
...
- [Setup/cleanup/restart cases](../../../internal/proxyinstallation/proxyinstallation_test.go#L1939), including precommit cleanup, committed setup finishing and every durable checkpoint.
- [Link-rotation recovery](../../../internal/proxyinstallation/proxyinstallation_test.go#L1379) and [enabled identity recovery](../../../internal/proxyinstallation/client_identity_subscription_test.go#L114).
- [Actual Certbot POSIX lock fixture](../../../internal/proxyinstallation/adapter/host/subscription_test.go#L42), [recorder publication tests](../../../internal/proxyinstallation/adapter/host/renewal_test.go#L433), and [renewal exclusion tests](../../../internal/proxyinstallation/adapter/host/renewal_test.go#L807).
- [Private dispatch/TLS composition](../../../internal/proxyinstallation/serving_test.go#L100), network tests beside `subscriptionserving/serving.go`, and Linux-specific sandbox tests.
...
The local [v3.1.69 report](../../acceptance/reports/v3.1.69-live-2026-09-12.md#L10) records **2 accepted, 1 failed, 22 blocked**. It records 307 helper tests and a 136-file bundle, followed by a `Prepared Action facts` refusal after an observed setup interruption; later supported cleanup completed. Those counts show that preparation did not predict this failure, not that every preparatory check was useless.

The public code has several possible refusal sites: [Finish cleanup's staged checkpoint recovery](../../../internal/proxyinstallation/proxyinstallation.go#L986) and [fresh authority/inspection comparison](../../../internal/proxyinstallation/proxyinstallation.go#L1023) both produce that message. The report alone does not identify the changed fact. Do not remove `reflect.DeepEqual` or weaken ownership checks solely to obtain a green scenario.
