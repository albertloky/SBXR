...
- `CERTIFICATE-RENEWAL-STATE`: `go test ./internal/state -run 'Test(PrepareIPCertificateRenewalCommitAllowsOnlyStandingScope|PrepareDomainCertificateRenewalCommitAllowsOnlyStandingScope|CertificateRenewalSchedulerUsesRealOneUseSystemChangesLock)'` covers each exact Desired State delta, separate planning after the real System Changes lock, serial IP-then-domain execution, one publication per success, durable Complete, cleanup, and both five-step transactions.
