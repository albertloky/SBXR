...
func TestRenewalAttemptStoreSurvivesSeparateSchedulerProcesses(t *testing.T) {
	...
	if err := first.StoreIPAttempt(now, certificatelifecycle.RenewalFailed); err != nil {
		t.Fatal(err)
	}
	second := newRenewalAttemptStore(root, os.Geteuid())
	at, outcome, found, err := second.LoadIPAttempt()
	...
	if err := second.ClearIPAttempt(); err != nil {
		t.Fatal(err)
	}
	if _, _, found, err := newRenewalAttemptStore(root, os.Geteuid()).LoadIPAttempt(); err != nil || found {
...
func TestRenewalAttemptStoreRejectsUnsafeOrMalformedHistory(t *testing.T) {
	...
	if _, _, _, err := store.LoadIPAttempt(); err == nil {
