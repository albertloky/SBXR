...
|---|---|---|---|
| Module Verification | Codex | Passed for authority, both routes, and installation | `CLOUDFLARE-MODULE`, `CLOUDFLARE-AUTHORITY`, `CLOUDFLARE-PLAN`, `CLOUDFLARE-XHTTP-ROUTE`, `CLOUDFLARE-WEBSOCKET-ROUTE`, `CLOUDFLARE-WHOLE-TUNNEL`, `CLOUDFLARE-NETWORK-PATH`, and `CLOUDFLARE-STATE-COMMIT` pass with marker-safe results. Token lifecycle, repair, and removal remain Pending. |
| Seam Verification — controlled fixture | Codex | Passed for authority, both routes, and installation | Official-shape HTTP, exact two-route publication, Plan freshness, provider steps, protected service installation and rollback, separate HTTP-origin proof, whole-Tunnel health, deferred State finalization, and restart-generation checks pass without an Owner credential. |
...
The implementation result is `Both Tunnel routes and whole-Tunnel health verified locally; Cloudflare Module and Release Qualification pending`. Real Cloudflare edge behavior, reconnect/keepalive, and outside access remain Pending. Issues `#88`–`#92` remain open for Direct DNS, token lifecycle, managed repair, and Complete removal. Unperformed rows stay Pending.
