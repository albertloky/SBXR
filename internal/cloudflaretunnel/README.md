...
Owned public exposure and the exact immutable-ID Cloudflare DNS, routes, and Tunnel are removed and verified while rollback is still possible and the scoped token remains active locally. System Changes then records `Owned external deletion verified` and `Irreversible removal started` immediately before SBXR asks the Owner to revoke that token in Cloudflare. From the irreversible record onward, cancellation and rollback are unavailable. SBXR proves that the still-protected local token is rejected, then removes the local copy and continues the fixed cleanup to `Not installed`.
