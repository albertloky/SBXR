...
This procedure covers issues #100–#107, #174, and #216: VLESS REALITY Vision, VLESS XHTTP, independent VLESS WebSocket, Hysteria2, TUIC, AnyTLS, their truthful staged six-profile registry, root-runtime contribution, and the coordinated profile lifecycle.
...
go test ./internal/connectionprofiles/... -run 'TestRegistryUsesEveryReviewedAlternativePort|TestRegistryPlanRejectsObsoleteAllSixProfileFreshInstallation|TestNotSetUpProfilesHaveNoIndividualOrRotationAction|TestRegistryPlansSingleAndCoordinatedCredentialReplacement|TestRegistryPlansOnlyAuthorizedForwardRepair|TestCoreUpdateRequalification|TestLiveProfileCheck'
```

Run the smallest complete staged-registry checks through `ViewRegistry`, `PlanRegistry`, and `Plan.Apply`:

```sh
go test ./internal/connectionprofiles/... -run 'Test(RevisionOneView|RevisionOnePlan|CloudflareSetupPlan|ObserveDeferredRegistry)' -count=1
go test -race ./internal/connectionprofiles/... -run 'Test(RevisionOneView|RevisionOnePlan|CloudflareSetupPlan|ObserveDeferredRegistry)' -count=1
go vet ./internal/connectionprofiles/...
```

These checks prove expected absence, independent deferred credentials and paths, complete native candidates, typed revision and Change Set binding, one-use State authority, System Changes admission, service and listener absence, atomic five-profile refusal, and secret-safe results. They do not run a real VPS, Cloudflare, certificate order, outside client, maintained client, Owner workflow, or Release Qualification.
...
- `CONNECTION-PROFILES-REGISTRY-EXPECTED-ABSENCE`, `-HEALTHY`, `-DISABLED`, `-EXPOSURE`, and `-PLAN-READY`: revision `1` has one Enabled REALITY profile and five exact `Not set up` profiles; later setup enables all five together with independent credentials and paths; safe readiness, selected listener, hostname, qualified-version, desired-enablement, and fresh-health facts; and rejection of partial setup or a seventh profile;
...
- reviewed fresh-install alternatives propagate the selected VLESS REALITY Vision port while every deferred setting and port stays absent. Cloudflare Profile Setup later propagates all five selected deferred ports together through native configurations, Network Policy exposure, active health, State service material, publication, and Cloudflare route health. Once committed, a later conflict stops in Network Policy's exact Correction Flow; it never silently selects another port;
...
25. On revision `1`, prove the registry contains exactly VLESS REALITY Vision Enabled plus VLESS XHTTP, VLESS WebSocket, Hysteria2, TUIC, and AnyTLS `Not set up`; prove every deferred setting, credential, service, listener, route, certificate binding, exposure, and publication entry absent. Then run one Cloudflare Profile Setup and prove all five become Enabled together with independent credentials and paths. Confirm VMess, every seventh profile, and every partial setup state are absent.
...
|---|---|---|---|
| Module Verification | Codex | Passed for #100–#107, #174, and #216 | Focused Connection Profiles and controlled Ubuntu checks cover revision `1` expected absence, the atomic five-profile transition, independent credentials and paths, native candidates, health, lifecycle changes, rollback input, one-use binding, typed publication handoff, and Live Profile Check privacy. |
...
Module accepted; Release Qualification pending. Revision `1` expected absence, one atomic five-profile setup contribution, the exact six-profile registry, complete native Xray and sing-box candidates, per-profile health and security bindings, reviewed disablement and re-enable, credential rotation, stable port correction, proven-lineage repair, rollback and restart resolution, typed publication handoff, core requalification input, and privacy-preserving optional Live Profile Check are verified locally. The full repository suite remains blocked by the pre-existing Installation composition failures owned by #219. Integrated Cloudflare Profile Setup composition remains owned by #220. Codex Live Acceptance, maintained-client and required Owner Acceptance, and Release Qualification remain Pending. Unperformed rows stay Pending.
