...
go test ./internal/installation -run '^TestInstallation(InterfaceOwnsPartialDraftUntilDiscard|InterfaceCancellationBeforeApplyDiscardsDraftAndApproval|ReviewOwnsFirstDraftDefaultsAndProgression|ReviewGuidesUnprovedAndAmbiguousNetworkFacts|ReviewDerivesAndImmediatelyChecksOneRealityHostname|ReviewShowsEveryExplicitPortReplacementInThePlan|ReviewKeepsTheRunningReleaseReadOnlyAfterCandidateFailure|ReviewSuppliesDomainHelpAndRejectsItsExample)$' -count=1
go test ./internal/ownerconsole -run '^TestRun(ShowsReadOnlyInstallationFactsAndDetectedPublicIPv4|ShowsTheSameCompleteDomainHelpAtExactTerminalSizes)$' -count=1
