...
go test ./internal/installation -run '^Test(Installation(InterfaceOwnsPartialDraftUntilDiscard|InterfaceCancellationBeforeApplyDiscardsDraftAndApproval|ReviewOwnsFirstDraftDefaultsAndProgression|ReviewGuidesUnprovedAndAmbiguousNetworkFacts|ReviewDerivesAndImmediatelyChecksOneRealityHostname|ReviewShowsEveryExplicitPortReplacementInThePlan|ReviewKeepsTheRunningReleaseReadOnlyAfterCandidateFailure|ReviewSuppliesDomainHelpAndRejectsItsExample|ReviewGuidesEveryFirstInstallationFieldAndRejectsTutorialValues)|ReclamationConfirmationGuidanceOwnsTheExactDestructiveBoundary)$' -count=1
go test ./internal/ownerconsole -run '^TestRun(ShowsReadOnlyInstallationFactsAndDetectedPublicIPv4|ShowsTheSameCompleteDomainHelpAtExactTerminalSizes|ShowsEveryInstallationHelpSourceAtExactTerminalSizes|FieldChangeClearsEditingStateThroughThePublicOutcome|MasksAndRevealsOnlyTheFocusedInstallationInfrastructureSecret|ResizeRemasksInitialAndManagedCloudflareTokens)$' -count=1
...
go test ./internal/softwarelifecycle -run '^TestPlanUpdatePreservesRealityOnlyCapabilityWithoutCloudflareWork$' -count=1
go test ./internal/state -run '^TestSoftwareLifecycleCapabilityProvesBothManagedProfileStates$' -count=1
...
Require one Plan to bind the current Release Identity, revision, State checksum, opaque State-owned capability proof, higher candidate, exact `1 → 2` migration, Connection Profiles and Subscription Publication contributions, and a Cloudflare Tunnel contribution only after Cloudflare Profile Setup. Before setup, the affected services are exactly `xray.service` and `sbxr-subscription.service`; installed inactive `sing-box.service` and `cloudflared.service` are replaced without activation, restart, or live Cloudflare checks. After setup, all four managed services are affected. Require one use, an exact post-approval recheck, one prepared revision `N → N+1`, and exact State/release/migration/Plan checksums before System Changes receives an `Update` Change Set. The only accepted migration performs no network access, adds the explicit profile lifecycle fields, preserves every typed Owner value and secret, and forces release-bound configuration and current subscription representations to regenerate and validate. A candidate without the exact qualified staged-capability schema and migration is refused during authenticated staging before mutation.
...
go test ./internal/softwarelifecycle -run '^TestPlanRepairPreservesRealityOnlyCapabilityWithoutProviderWork$' -count=1
...
Require one deterministic `PlanRepair` to consume the fresh observation, opaque State-owned capability proof, and exactly one typed owning-Module contribution. A reality-only State refuses a Cloudflare Tunnel repair contribution. Bind the unchanged current Owner intent, exact reversible steps, required pre- and post-publication checks, disk, rollback, and affected Module without rendering protected material. Apply must consume the Plan once, recheck State, capability, observation, and contribution after approval, accept only State's exact unchanged-intent revision `N → N+1`, and hand one `Repair` Change Set to System Changes. Required failure, cancellation, process death, or disagreement uses the normal transaction rollback and restart rules; unprovable resolution remains Recovery Required.
...
### SL-REMOVAL-08 — Complete removal remains rollback-safe through required exposure removal

Require Software Lifecycle-owned confirmation Help to name the exact deletion authority, irreversible boundary, and consequence of `COMPLETE REMOVAL`. Help creates no phrase, selection, Plan approval, or Apply authority.

Run:

```sh
go test ./internal/softwarelifecycle -run 'CompleteRemoval' -count=1
go test ./internal/softwarelifecycle -run '^TestPlanCompleteRemovalBeforeCloudflareSetupContainsOnlyProvedLocalWork$' -count=1
go test ./internal/systemchanges -run 'CompleteRemoval' -count=1
go test ./internal/state -run '^TestCompleteRemovalWithoutCloudflareFinishesWithoutProviderOrTokenWork$' -count=1
go test ./internal/state -run 'CompleteRemoval|Removal' -count=1
```

Require `ViewCompleteRemoval` to expose one protected `Review Complete removal` candidate only from a fresh Managed or Recovery Required observation with volatile facts, a released lock, and no unfinished Change Set or automatic rollback. Bind exact valid State lineage and its opaque capability proof when available; raw bytes or absence may be preserved only as the exact rollback baseline and must never become Cloudflare ownership or token authority. Before Cloudflare Profile Setup, require only the full local inventory and exact Network Policy public resources. The Plan and Apply contain no token entry, provider deletion, provider verification, or token-revocation step. After setup, also require exact Cloudflare DNS, route, and Tunnel identifiers and active locally available token authority. In both states, require disk, ordinary sudo, irreversible remnants, cancellation boundary, rollback to the exact starting status, and final Not installed proof without rendering protected values. Refuse stale or reused observations, missing required categories, caller-made reviews, changed identifiers, incomplete ownership, unavailable required token authority, or a competing unfinished transaction. A Recovery Required review without independent immutable-ID and token proof returns a typed refusal with safe diagnostics, manual provider-cleanup, and Clean-VPS rebuild guidance instead of adopting raw State.

Apply must require the genuine exact interactively typed `COMPLETE REMOVAL` authority and the separate genuine `Permanently remove SBXR` selection, recheck every fact after approval, accept only State's exact rollback binding, and hand one Complete-removal Change Set to System Changes. Failure, cancellation, or restart through required public-exposure removal, plus Tunnel-route removal only after Cloudflare Profile Setup, restores Managed to Managed and Recovery Required to Recovery Required unless transaction-bound State genuinely proves the exact earlier Managed baseline. Before setup, local readiness directly authorizes transaction-material deletion without any provider checkpoint. After setup, record `Irreversible removal started` only after the rollback-capable absence gate and local readiness pass; only then delete exact immutable-ID DNS records and the Tunnel, record `Owned external deletion verified`, delete the exact freshly observed management-token ID, and prove that token unauthorized. Issue #133 owns every forward-only phase after that checkpoint. External Integrated Verification, Codex Live Acceptance, Owner Acceptance, and Release Qualification remain Pending on issue #127 and the inherited production prerequisites.
...
After rollback-capable public and Tunnel-route absence is durable, require exact local-deletion readiness before recording `Irreversible removal started`. Delete the immutable-ID-proven DNS records and Tunnel forward-only, record `Owned external deletion verified`, durably record the exact freshly observed management-token ID, and keep the local token copy protected while Cloudflare Tunnel deletes that ID and proves the token unauthorized. A failed, uncertain, or still-authorized result reports the last durable forward checkpoint and retries automatically through the protected recovery runner. A process death after provider deletion accepts explicit unauthorized proof only against the previously recorded ID. It never asks Albert to revoke the token and never creates a revocation-unverified state.
