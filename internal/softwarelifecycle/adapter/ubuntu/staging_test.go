...
func stageRequest(t *testing.T, architecture softwarelifecycle.Architecture, amd64Archive, arm64Archive []byte) softwarelifecycle.StageRequest {
	...
	index := []byte(fmt.Sprintf(`{"schema":1,"product":"sbxr","repository":"albertloky/SBXR","version":"1.0.0","sequence":1,"tag":"v1.0.0","commit":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","state_schema":2,"minimum_updater_schema":1,"assets":[{"role":"application-linux-amd64","name":"sbxr-linux-amd64.tar.gz","size":%d,"sha256":"%s"},{"role":"application-linux-arm64","name":"sbxr-linux-arm64.tar.gz","size":%d,"sha256":"%s"},{"role":"components-linux-amd64","name":"sbxr-components-linux-amd64.tar.gz","size":%d,"sha256":"%s"},{"role":"components-linux-arm64","name":"sbxr-components-linux-arm64.tar.gz","size":%d,"sha256":"%s"},{"role":"bootstrap","name":"install.sh","size":%d,"sha256":"%s"}]}`, len(amd64Archive), hex.EncodeToString(amd64Digest[:]), len(arm64Archive), hex.EncodeToString(arm64Digest[:]), len(amd64Components), hex.EncodeToString(amd64ComponentsDigest[:]), len(arm64Components), hex.EncodeToString(arm64ComponentsDigest[:]), len(bootstrap), hex.EncodeToString(bootstrapDigest[:])))
	indexDigest := sha256.Sum256(index)
	evidence := softwarelifecycle.ReleaseEvidence{Repository: softwarelifecycle.Repository, Tag: "v1.0.0", Commit: strings.Repeat("a", 40), Index: index, Assets: []softwarelifecycle.DownloadedAsset{{Name: "sbxr-linux-amd64.tar.gz", Bytes: amd64Archive}, {Name: "sbxr-linux-arm64.tar.gz", Bytes: arm64Archive}, {Name: "sbxr-components-linux-amd64.tar.gz", Bytes: amd64Components}, {Name: "sbxr-components-linux-arm64.tar.gz", Bytes: arm64Components}, {Name: "install.sh", Bytes: bootstrap}}, AttestedAssets: []softwarelifecycle.AttestedAsset{{Name: "release-index.json", SHA256: hex.EncodeToString(indexDigest[:])}, {Name: "sbxr-linux-amd64.tar.gz", SHA256: hex.EncodeToString(amd64Digest[:])}, {Name: "sbxr-linux-arm64.tar.gz", SHA256: hex.EncodeToString(arm64Digest[:])}, {Name: "sbxr-components-linux-amd64.tar.gz", SHA256: hex.EncodeToString(amd64ComponentsDigest[:])}, {Name: "sbxr-components-linux-arm64.tar.gz", SHA256: hex.EncodeToString(arm64ComponentsDigest[:])}, {Name: "install.sh", SHA256: hex.EncodeToString(bootstrapDigest[:])}}, Verifier: softwarelifecycle.VerifierEvidence{Version: "2.97.0", SigningFingerprint: strings.Repeat("A", 40), OfficialSignedDistribution: true, ReleaseVerified: true, VerifiedAssets: []string{"release-index.json", "sbxr-linux-amd64.tar.gz", "sbxr-linux-arm64.tar.gz", "sbxr-components-linux-amd64.tar.gz", "sbxr-components-linux-arm64.tar.gz", "install.sh"}}}
