...
Pass when the `Load` Interface proves clean absence, valid Managed and Change in progress snapshots, a complete typed Desired State, and revision `1` with VLESS REALITY Vision `Enabled` plus exactly five `Not set up` profiles. Revision `1` must omit all deferred settings, credentials, Cloudflare facts, domain-certificate facts, listeners, routes, exposure, and publication entries. The check also proves exact field names, duplicate-key and unknown-field refusal, supported Release Identity, checksum integrity, every lineage outcome, cross-section validation, complete secret-safe findings, and marker absence. This check owns the `Load` and Correction Flow behavior.
