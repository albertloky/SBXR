...
- `SUBSCRIPTION-PUBLICATION-SERVING-BOUNDARY`: the real `current/` directory contains only `serving.json`, the seven named bodies, and metadata at `0640` under the sole group-traversable `0750` directory; `sets/` and inactive generations are root-only `0700`, parent, prepared-configuration, and final symlinks are refused, and Subscription Serving receives no Desired State, renderer, typed source, validator, or arbitrary file path.
