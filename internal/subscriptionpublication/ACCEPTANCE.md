...
- `SUBSCRIPTION-PUBLICATION-ATOMIC-ACTIVATION`: the filesystem Adapter writes the protected minimum serving configuration beside the eight Publication files in one root-only candidate generation and atomically exchanges it into the real `/var/lib/sbxr/subscriptions/current/` directory on supported Ubuntu targets; readers observe the complete prior authorization-and-body set or complete candidate, never a mixture.
...
- `SUBSCRIPTION-PUBLICATION-SERVING-BOUNDARY`: the real `current/` directory contains only `serving.json`, the seven named bodies, and metadata as `root:root 0644` under the exact `root:root 0755` directory; `sets/` and inactive generations are non-traversable outside root at `0700`, parent, prepared-configuration, and final symlinks are refused, and Subscription Serving receives no Desired State, renderer, typed source, validator, or arbitrary file path.
