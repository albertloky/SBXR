...
Subscription Publication turns one complete typed `connectionprofiles.PublicationSource` into client artifacts. The current slices own six representations:
...
- `Karing`: the same validated sing-box JSON bytes, with metadata that reports VLESS XHTTP as `Not offered` because it is unavailable in the Karing core.
...
Atomic artifact activation, serving, token rotation, Shadowrocket support, and maintained-client compatibility claims belong to later slices of issue #108.
