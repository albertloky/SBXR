...
`Plan` renders and validates all seven representations, adds one metadata file, and binds that complete candidate to revision `N+1`, the starting Desired State lineage, relevant managed-input checksums, `sbxr-subscription-representations-v1`, the selected address, the exact typed Connection Profile source, and the Release Identity. Its summary reports only identities, counts, omissions, validation status, complete replacement, and rollback intent. Bodies, tokens, Client Access Values, complete URLs, and secret-derived artifact checksums remain protected.

`Apply` is one-use and contributes one reversible Subscription Publication step to the existing System Changes transaction. State carries one opaque, SHA-256-bound bundle owned by Subscription Publication; that bundle validates and enumerates the exact eight protected files. The filesystem Adapter rejects hostile parent or final symlinks, prepares root-only generations under `/var/lib/sbxr/subscriptions/sets/`, then atomically exchanges a complete candidate into the real `/var/lib/sbxr/subscriptions/current/` directory on the supported Ubuntu `linux/amd64` and `linux/arm64` targets. `sets/` and every inactive generation remain root-only `0700`; only `current/` is group-traversable `0750`. Subscription Serving therefore observes either the complete prior set or complete candidate and cannot traverse a generation path. System Changes runs Required candidate and activation checks, publishes Desired State exactly once, runs the post-publication Subscription Serving agreement check, records durable `Complete`, and only then removes older generations.

Any rendering, validation, preparation, storage, activation, pre-publication, post-publication, or Subscription Serving health failure fails closed. Before activation, nothing changes. After activation, the exact prior directory and Desired State are restored from durable evidence. Restart inspection treats an inactive candidate directory as recoverable transaction work; unprovable lineage or rollback becomes `Recovery Required`. Subscription Serving receives only the immutable named artifacts plus its separate State-prepared listener credential, certificate pointer, port, and selected address. It does not receive Desired State, Connection Profile credentials outside the rendered bodies, validators, renderer access, the typed source, or arbitrary file access.

Subscription Serving implementation, token rotation, Shadowrocket support claims, and maintained-client compatibility claims remain later slices and acceptance work under issue #108.
