...
- `SYSTEM-CHANGES-RECLAMATION`: `go test ./internal/state ./internal/systemchanges/adapter/ubuntu -run 'Test(ReclamationInstallation|DeferredCloudflareFinalization|ReclamationDigest)'`
...
2. Under the existing global lock and protected transaction snapshot, bind the Plan to each owning Module's complete immutable-ID inventory for all six reviewed categories. Require the typed steps to cover those inventories exactly, including multiple resources in one category and typed empty-category evidence. Remove and verify typed owned public exposure and Tunnel routes before the irreversible boundary. Preserve the DNS records, Tunnel, unrelated resources, and active local scoped token. Never place the token in process arguments, environment, journal evidence, or typed results.
3. Fail or cancel before Apply and after each public or Tunnel-route safe checkpoint. From a proven Managed start, restore Managed. From a proven Recovery Required start, restore that exact status. Reverse only attempted typed resources in descending order and remove resolved transaction material.
4. End the process before `Irreversible removal started` and construct a fresh System Changes instance over the protected journal. Confirm recovery reverses every recorded public resource and Tunnel-route change, preserves the still-available scoped token, DNS records, Tunnel, and unrelated resources, and proves the exact starting status. Keep destructive VPS and real-token checks Pending without an approved Acceptance Run.

## SC-09 procedure

1. Remove and verify only the exact committed public exposure and Tunnel routes while protected rollback remains available. A changed identifier, route failure, absence failure, cancellation, or process death before the checkpoint restores the exact proven starting status without deleting DNS records or the Tunnel.
2. After the required rollback-capable absence gate passes, require the Ubuntu removal owner to prove that the fixed remaining sequence and ownership evidence are ready, then record `Irreversible removal started`. A readiness failure rolls back while that remains possible. After the irreversible record, delete the exact immutable-ID DNS records and Tunnel, durably record both phases and `Owned external deletion verified`, then ask the Owner to revoke the scoped Cloudflare token. Cancellation, Retry automatic rollback, and restore cannot reverse or restore anything after the irreversible record; recovery can only continue forward.
...
4. Continue in this fixed order: local State, Infrastructure Secrets, certificates, durable `Transaction material deletion authorized`, transaction material except the journal, releases, units, identities, listeners, prepared artifacts, owned firewall state, and finally the removal journal and private recovery runner. The authorization record makes a crash during protected snapshot/manifest deletion resumable; record durable evidence after every deletion phase. Repeat an uncertain phase idempotently; skip it only when its completion record is durable.
...
1. Admit reclamation only from Network Policy's one-use exact reviewed executable or supported-script authority and a fresh privileged recheck. Prepare and synchronize the complete transaction, create the reversible Cloudflare resources, finalize the provider values into protected State material, and durably record `State finalized` before `Irreversible reclamation started`.
2. Before the irreversible record, cancellation or any failure reverses attempted work and restores Not installed. After it, ignore cancellation, stop only the exact PID whose executable agrees with the reviewed executable or interpreter, record `Reclamation process stopped`, delete only the exact unchanged regular single-link unmounted target, and record `Reclamation target deleted`.
3. Refuse a changed PID, executable, interpreter, digest, pathname, symlink, mount, hard link, directory, or protected foundation target. Supported scripts use only the exact direct absolute interpreter and absolute script form admitted by Network Policy.
4. End the worker before and after every reclamation checkpoint and again during recovery. A fresh recovery runner resumes the exact next unproved forward phase from protected State and journal evidence, never reverses after the irreversible record, and reports success only after Managed agreement, durable `Complete`, and transaction cleanup.

## Current status

| Stage | Status | Evidence |
|---|---|---|
| Module Verification | Passed | SC-01 checks cover four-state inspection and safe admission. SC-02 checks cover durable success. `SYSTEM-CHANGES-ROLLBACK` covers SC-03 cancellation and failure. `SYSTEM-CHANGES-FORWARD-DEATH` and `SYSTEM-CHANGES-SERVICE-START` cover SC-04 restart rollback. SC-05 checks cover interrupted rollback continuation, a second death, exact service holdback, cleanup-only restart after `Complete`, and corrupt-evidence refusal. SC-06 checks cover the Recovery Required action boundary, exact retry eligibility, refusal, secret safety, and fresh forward repair. SC-07 checks cover typed firewall admission, watchdog ordering, non-default SSH proof, exact HTTP-01 identity and cleanup, interrupted reverse, restart recovery, unrelated-rule preservation, and renewal serialization ownership. SC-08 checks cover exact two-part confirmation, typed reversible public and immutable-ID external deletion, token availability, cancellation and failure at every logical boundary, both starting statuses, durable handoff, and pre-checkpoint restart rollback. SC-09 checks cover the irreversible boundary, verified token revocation, refusal of rollback and restore, fixed forward deletion, checkpoint restart continuation, final Not installed observation, and recovery-material cleanup. SC-10 checks cover the owner-assisted run-token checkpoint, protected replacement, controlled restart, both-route gates, and forward-only recovery. SC-11 checks cover exact reclamation authority, durable provider-valued recovery material, process stop, target deletion, checkpoint restart continuation, cancellation boundary, and forward-only completion. |
