...
- `SYSTEM-CHANGES-REVERSIBLE-REMOVAL`: `go test ./internal/state -run 'Test(CompleteRemovalDurablyCrossesIrreversibleBoundaryAfterExternalProof|UbuntuAdapterReversesOnlyTypedCompleteRemovalResourcesAfterRestart|UbuntuAdapterRefusesRemovalRollbackAfterScopedTokenDisappears)'`
...
- `SYSTEM-CHANGES-IRREVERSIBLE-CHECKPOINT`: `go test ./internal/state -run 'Test(CompleteRemovalDurablyCrossesIrreversibleBoundaryAfterExternalProof|IrreversibleCompleteRemovalRefusesAnAutomaticRollbackRequest)'`
...
2. Under the existing global lock and protected transaction snapshot, bind the Plan to each owning Module's complete immutable-ID inventory for all six reviewed categories. Require the typed steps to cover those inventories exactly, including multiple resources in one category and typed empty-category evidence. Remove and verify typed owned public exposure before deleting typed Cloudflare resources. Preserve unrelated resources, and keep the scoped token active and its local copy available throughout this reversible phase. Never place the token in process arguments, environment, journal evidence, or typed results.
3. After all owned external deletion is verified, durably record `Owned external deletion verified`, then let SC-09 durably cross `Irreversible removal started` before asking the Owner to revoke the token. Stop before Desired State publication, token revocation proof, local secret deletion, or any later irreversible work. Report that Certificate Transparency entries and DNS caches cannot be erased.
4. Fail each public-exposure step, external deletion, and absence check. Request cancellation before Apply, after each safe step checkpoint, and during final verification. From a proven Managed start, restore Managed. Refuse lineage-less Recovery Required before live work; when exact protected transaction evidence proves its earlier Managed baseline, restore Managed. Reverse only attempted typed resources in descending order and remove resolved transaction material.
5. End the process after typed external deletion and construct a fresh System Changes instance over the protected journal. Confirm recovery inspects and reverses only the recorded immutable resources, preserves the still-available Cloudflare token and unrelated resources, proves the exact starting status, and never continues forward. Keep destructive VPS and real-token checks Pending without an approved Acceptance Run.

## SC-09 procedure

1. Immediately after durable `Owned external deletion verified`, record `Irreversible removal started` before asking the Owner to revoke the scoped Cloudflare token. Before that record, failure may roll back from protected evidence. After it, cancellation, Retry automatic rollback, and restore cannot reverse or restore anything; recovery can only wait for revocation or continue forward.
2. Verify through the still-present local token copy that Cloudflare rejects the revoked token. Do not delete that local copy or any later resource until the verification passes. A usable token waits for Owner action; missing or unprovable token evidence enters forward-only Recovery Required and never rollback.
3. Continue in this fixed order: remaining owned external resources, local State, Infrastructure Secrets, certificates, services, identities, listeners, firewall rules, releases, transaction material, and finally the removal journal and private recovery runner. Record durable evidence after every deletion phase. Repeat an uncertain phase idempotently; skip it only when its completion record is durable.
4. End the worker immediately before and after every irreversible deletion checkpoint. Construct a fresh System Changes instance and confirm it resumes at the exact next unproven forward phase, never starts affected services, never runs a reverse operation, and never restores prior State.
5. Report success only after the production Adapter observes exact `Not installed`, every owned resource is absent, the local token copy is absent, the snapshot and prior release are absent, the transaction manifest and artifacts are absent, and finalization removes the journal and recovery runner. The controlled Ubuntu seam proves ordering, restart-safe evidence, and final cleanup; destructive VPS and real-token checks remain Pending without an approved Acceptance Run.

## Current status

| Stage | Status | Evidence |
|---|---|---|
| Module Verification | Passed | SC-01 checks cover four-state inspection and safe admission. SC-02 checks cover durable success. `SYSTEM-CHANGES-ROLLBACK` covers SC-03 cancellation and failure. `SYSTEM-CHANGES-FORWARD-DEATH` and `SYSTEM-CHANGES-SERVICE-START` cover SC-04 restart rollback. SC-05 checks cover interrupted rollback continuation, a second death, exact service holdback, cleanup-only restart after `Complete`, and corrupt-evidence refusal. SC-06 checks cover the Recovery Required action boundary, exact retry eligibility, refusal, secret safety, and fresh forward repair. SC-07 checks cover typed firewall admission, watchdog ordering, non-default SSH proof, exact HTTP-01 identity and cleanup, interrupted reverse, restart recovery, unrelated-rule preservation, and renewal serialization ownership. SC-08 checks cover exact two-part confirmation, typed reversible public and immutable-ID external deletion, token availability, cancellation and failure at every logical boundary, both starting statuses, durable handoff, and pre-checkpoint restart rollback. SC-09 checks cover the irreversible boundary, verified token revocation, refusal of rollback and restore, fixed forward deletion, checkpoint restart continuation, final Not installed observation, and recovery-material cleanup. |
