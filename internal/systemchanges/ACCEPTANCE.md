...
- `SYSTEM-CHANGES-REVERSIBLE-REMOVAL`: `go test ./internal/state -run 'Test(CompleteRemovalStopsAtTheReversibleExternalDeletionBoundary|UbuntuAdapterReversesOnlyTypedCompleteRemovalResourcesAfterRestart|UbuntuAdapterRefusesRemovalRollbackAfterScopedTokenDisappears)'`
...
3. After all owned external deletion is verified, durably record `Owned external deletion verified` and stop before Desired State publication, token revocation, local secret deletion, or any irreversible work. Report that Certificate Transparency entries and DNS caches cannot be erased. SC-09 alone may cross the irreversible checkpoint.
...
| Integrated Verification | Pending — integrated release | SC-09 irreversible continuation, all-Module wiring, Certificate Lifecycle unit installation/private-command wiring, and complete executable removal do not exist yet. Controlled SC-04 through SC-08 checks do not satisfy integrated verification. |
