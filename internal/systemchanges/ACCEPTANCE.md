...
- `SYSTEM-CHANGES-REVERSIBLE-REMOVAL`: `go test ./internal/state -run 'Test(CompleteRemovalDurablyCrossesIrreversibleBoundaryBeforeCloudflareDeletion|UbuntuAdapterReversesOnlyTypedCompleteRemovalResourcesAfterRestart)'`
...
- `SYSTEM-CHANGES-IRREVERSIBLE-CHECKPOINT`: `go test ./internal/state -run 'Test(CompleteRemovalDurablyCrossesIrreversibleBoundaryBeforeCloudflareDeletion|CompleteRemovalFailureAfterTheIrreversibleBoundaryResumesForward|IrreversibleCompleteRemovalRefusesAnAutomaticRollbackRequest)'`
...
4. Confirm the Certificate Lifecycle-owned scheduling contract names exactly one persistent randomized service and one timer, evaluates only the IP lineage at least twice daily, creates one one-use Change Set when due, and sends every Apply through the global lock. Hold the lock and confirm the renewal is deferred without a queue or Plan construction; after release, the scheduler must rebuild observations and a fresh Plan under that lock within its own due and retry policy. The domain lineage remains pending issue #98.
...
2. Under the existing global lock and protected transaction snapshot, bind the Plan to each owning Module's complete immutable-ID inventory for all six reviewed categories. Require the typed steps to cover those inventories exactly, including multiple resources in one category and typed empty-category evidence. Remove and verify typed owned public exposure before the irreversible boundary. Preserve unrelated resources, and keep the scoped token active and its local copy available. Never place the token in process arguments, environment, journal evidence, or typed results.
3. Fail or cancel before Apply and after each public-exposure safe checkpoint. From a proven Managed start, restore Managed. Refuse lineage-less Recovery Required before live work; when exact protected transaction evidence proves its earlier Managed baseline, restore Managed. Reverse only attempted typed public resources in descending order and remove resolved transaction material.
4. End the process before `Irreversible removal started` and construct a fresh System Changes instance over the protected journal. Confirm recovery reverses only recorded public resources, preserves the still-available Cloudflare token and unrelated resources, and proves the exact starting status. Keep destructive VPS and real-token checks Pending without an approved Acceptance Run.

## SC-09 procedure

1. Record `Irreversible removal started` immediately before the first permanent Cloudflare deletion. Before that record, failure may roll back from protected evidence. After it, cancellation, Retry automatic rollback, and restore cannot reverse or restore anything; recovery can only continue forward.
2. Delete and verify only the exact committed Cloudflare identifiers. After the required absence gate passes, record `Owned external deletion verified`, then ask the Owner to revoke the scoped Cloudflare token. A changed identifier stops before the checkpoint; any deletion or absence failure after it enters forward-only Recovery Required.
